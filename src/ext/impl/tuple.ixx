...
_EXT_BEGIN
    ...
    {return detail::make_from_tuple<T>(std::forward<Tuple>(tuple), std::make_index_sequence<std::tuple_size_v<std::remove_reference_t<Tuple>>>{});}
    ...
    {detail::tuple_foreach(std::make_index_sequence<std::tuple_size_v<std::decay_t<T>>>{}, std::forward<T>(tuple), std::forward<F>(function), std::forward<Iterable>(iterable));}
