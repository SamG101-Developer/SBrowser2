...
public:
    explicit bind_back(const F& function, Ts&&... fixed_args)
            : m_function{function}
            , m_back_args{ext::make_tuple(std::forward<Ts>(fixed_args)...)}
    {
    ...
    constexpr auto operator()(Ts1&&... variable_args) const
    {
        ...
        auto m_front_args = ext::make_tuple(std::forward<Ts1>(variable_args)...);
...
public:
    explicit bind_front(const F& function, Ts&&... fixed_args)
            : m_function{function}
            , m_front_args{ext::make_tuple(std::forward<Ts>(fixed_args)...)}
    {
    ...
    constexpr auto operator()(Ts1&&... variable_args) const
    {
        ...
        auto m_back_args = ext::make_tuple(std::forward<Ts1>(variable_args)...);
        return ext::apply(m_function, ext::tuple_cat(m_front_args, m_back_args));
    }
