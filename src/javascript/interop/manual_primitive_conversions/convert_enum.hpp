...
template <ext::type_is_enum T>
inline auto v8pp::convert<T>::is_valid(v8::Isolate* isolate, v8::Local<v8::Value> v8_value) -> ext::boolean
{
...
template <ext::type_is_enum T>
auto v8pp::convert<T>::from_v8(v8::Isolate* isolate, to_type v8_value) -> from_type
{
    ...
    auto cpp_value = v8pp::convert<ext::string>::from_v8(isolate, v8_value.template As<v8::String>());
    auto cpp_value_formatted = cpp_value.empty() ? u8"_" : cpp_value
            | ranges::views::replace('-', '_')
            | ranges::views::uppercase
            | ranges::to<ext::string>;

    auto cpp_value_enum = magic_enum::enum_cast<T>(std::move(cpp_value_formatted));
...
template <ext::type_is_enum T>
auto v8pp::convert<T>::to_v8(v8::Isolate* isolate, from_type cpp_value) -> to_type
{
    v8::EscapableHandleScope javascript_scope{isolate};

    auto cpp_value_formatted = ext::string{magic_enum::enum_name(cpp_value)} == u8"_" ? u8"" : cpp_value
            | ranges::views::replace('_', '-')
            | ranges::views::lowercase
            | ranges::to<ext::string>;

    auto v8_value_string = v8::String::NewFromUtf8(isolate, cpp_value_formatted.c_str()).ToLocalChecked();
