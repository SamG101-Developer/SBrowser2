        ...
        -> void
{
    ...
    {
        video_element->d_func()->total_video_frame_count = 0;
        video_element->d_func()->m_dropped_video_frame_count = 0;
    }
    ...
    decltype(auto) pending_tasks = ext::vector<detail::task_t>{element->d_func()->media_element_event_task_source};
    pending_tasks
            | ranges::views::filter([](auto&& task) {return false; /* TODO */})
            | ranges::views::for_each(ext::invoke{})
            | ranges::views::for_each([](auto&& task) {element->d_func()->media_element_event_task_source |= ranges::actions::remove(task);});
    ...
    if (element->d_func()->network_state == element->NETWORK_IDLE || element->d_func()->network_state == element->NETWORK_LOADING)
        dom::detail::queue_media_element_task(element, BIND_FRONT(dom::detail::fire_event, u8"abort", element));
    ...
    if (element->d_func()->network_state != element->NETWORK_EMPTY)
    {
        ...
        dom::detail::queue_media_element_task(element, BIND_FRONT(dom::detail::fire_event, u8"emptied", element));
        ...
        if (ext::holds_alternative<media::source::media_source*>(element->d_func()->assigned_media_provider_object))
            media::detail::detach_from_media_element(ext::get<media::source::media_source*>(element->d_func()->assigned_media_provider_object));
        ...
        element->d_func()->ready_state = element->HAVE_NOTHING;
        ...
        if (!element->d_func()->paused())
        {
            element->d_func()->media_resource->pause();
            auto promises = take_pending_play_promises(element);
            reject_pending_play_promises(take_pending_play_promises(element), dom::other::dom_exception{u8"", ABORT_ERR});
        }

        // Ensure that the HTMLMediaElement isn't seeking
        element->d_func()->seeking = false;
        ...
        element->d_func()->current_playback_position = 0;
        element->d_func()->official_playback_position = 0;
        if (watch_official_position.has_changed())
            dom::detail::queue_media_element_task(element, BIND_FRONT(dom::detail::fire_event, u8"timeupdate", element));

        // TODO : timeline offset

        // Set the duration of teh HTMLMEdiaElement to NaN (duration is unknown whilst the new resource is being
        // fetched).
        element->d_func()->duration = ext::number<double>::nan();
    }
    ...
    element->d_func()->media_resource->setPlaybackRate(element->d_func()->default_playback_rate);
    element->d_func()->error = nullptr;
    element->d_func()->can_autoplay_flag = true;
        ...
        -> void
{
    ...
    auto mode = ext::holds_alternative<dom_object*>(url_or_media_provider)
    ...
    element->d_func()->tracks |= ranges::actions::filter(is_media_resource_specific_text_track);

    if (mode == REMOTE)
    {
        {
            element->d_func()->network_state = element->NETWORK_IDLE;
            dom::detail::queue_media_element_task(element, [element] {dom::detail::fire_event(u8"suspend", element);});
            dom::detail::queue_media_element_task(element, [element] {element->d_func()->delaying_load_event_flag = false;});

            // TODO : wait for task
            // TODO : wait for event
            element->d_func()->delaying_load_event_flag = true;
            element->d_func()->network_state = element->NETWORK_LOADING;
        }
        ...
        decltype(auto) request = html::detail::create_potential_cors_request(current_media_resource, destination, element->d_func()->cross_origin);
        decltype(auto) document = element->d_func()->node_document;
    ...
    else if (mode == LOCAL)
    {
                ...
                || dynamic_cast<media::source::media_source_handle*>(ext::get<dom_object*>(url_or_media_provider)))
        {
            ...
            else if (handle && handle->d_func()->detached())
                media_data_processing_steps_list(element, detail::media_data_processing_t::CANNOT_BE_FETCHED);

            else if (handle->d_func()->source->has_ever_been_detached())
                media_data_processing_steps_list(element, detail::media_data_processing_t::CANNOT_BE_FETCHED);

            else if (source->d_func()->ready_state != media::detail::ready_state_t::CLOSED)
                media_data_processing_steps_list(element, detail::media_data_processing_t::CANNOT_BE_FETCHED);

            else
            {
                source->d_func()->has_event_been_detached = true;
                element->d_func()->delaying_load_event_flag = false;
                ...
                {
                    element->d_func()->channel_with_worker = std::make_unique<html::messaging::message_channel>();
                    element->d_func()->port_to_worker = element->d_func()->channel_with_worker()->port1();
                    auto serialized_port2 = html::detail::structured_serialize_with_transfer(element->d_func()->channel_with_worker()->port2());
                    ...
                    dom::detail::queue_task(dedicated_worker_global_scope->m_task_queue, [source, dedicated_worker_global_scope]
                    {
                        ...
                        source->d_func()->port_to_main = element->d_func()->channel_with_worker(); // TODO: deserialized
                        source->d_func()->ready_state = media::detail::ready_state_t::OPEN;
                        dom::detail::queue_task(dedicated_worker_global_scope->m_task_queue,
                                [source, dedicated_worker_global_scope]
                                {dom::detail::fire_event(u8"sourceopen", source);});
                    });
                }

                else if (decltype(auto) window = v8pp::from_v8<dom::nodes::window*>(source_creation_agent, source_creation_global_object))
                {
                    element->d_func()->channel_with_worker = nullptr;
                    element->d_func()->port_to_worker = nullptr;
                    source->d_func()->port_to_main = nullptr;
                    source->d_func()->ready_state = media::detail::ready_state_t::OPEN;
                    dom::detail::queue_task(dedicated_worker_global_scope->m_task_queue,
                            [source, dedicated_worker_global_scope]
                            {dom::detail::fire_event(u8"sourceopen", source);});
