        ...
        -> void
{
...

    {
        ...
        for_if (form_associated, auto callback_name: {u8"formAssociatedCallback", u8"formResetCallback", u8"formDisabledCallback", u8"formStateRestoreCallback"})
        {
            auto callback_value = v8_prototype.As<v8::Object>()->Get(this_relevant_realm, v8pp::to_v8(this_relevant_agent, callback_name)).ToLocalChecked();
            auto callback = v8pp::from_v8<dom::detail::lifecycle_callback_t>(this_relevant_agent, callback_value);
            lifecycle_callbacks.try_emplace(callback_name, std::move(callback));
    ...
    decltype(auto) definition = std::make_unique<dom::detail::custom_element_definition_t>(
            ...
            form_associated, disabled_itnernals, disable_shadow);

    d->custom_element_definitions.emplace_back(definition);

    decltype(auto) document = v8pp::from_v8<dom::nodes::window*>(this_relevant_agent, this_relevant_global_object)->d_func()->document;
    decltype(auto) upgrade_candidates = dom::detail::shadow_including_descendants(document)
            | ranges::views::filter_eq(&dom::nodes::node_private::namespace_, dom::detail::HTML, ext::get_pimpl)
            | ranges::views::filter_eq(&dom::nodes::node_private::local_name, local_name, ext::get_pimpl)
            | ranges::views::filter([extends](dom::nodes::node* child) {return extends.empty() ? true : child->d_func()->is == extends;});
}
