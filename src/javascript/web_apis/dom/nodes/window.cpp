        ...
        -> ext::number<ulong>
{
    ...
    std::jthread this_thread(
            [this, options = std::move(options), &handle, &this_thread]
            {
                ...
                if (auto timeout = options.try_emplace("timeout", 0).first->second.to<ext::number<int>>(); timeout > 0)
                {
                    ...
                    m_condition_variable.wait(
                            lock, [this, &this_thread] {return &*m_current_threads.unsafe_begin() == &this_thread;});
        ...
        -> void
{
    ...
    m_idle_request_callbacks  |= ranges::actions::remove_at_index(std::move(handle));
    m_runnable_idle_callbacks |= ranges::actions::remove_at_index(std::move(handle));
}
