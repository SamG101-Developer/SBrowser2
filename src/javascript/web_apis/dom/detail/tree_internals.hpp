        ...
        -> nodes::node*
{
    ...
    return not node ? nullptr :
           node->parent_node ? root(node->parent_node) : node;
}
        ...
        -> range_v3_view auto
{
    ...
    ext::vector<nodes::node*> ancestor_nodes {node};
    while ((node = node->parent_node)) ancestor_nodes.push_back(node);
        ...
        -> range_v3_view auto
{
    ...
    ext::vector<nodes::node*> descendant_nodes {node};
    for (const auto& child: descendant_nodes) descendant_nodes.extend(ranges::find(descendant_nodes, child), *(ext::vector<nodes::node*>*)child->child_nodes);
        ...
        -> ext::boolean
{
    ...
    return ranges::find(ancestors_of_b, node_a) != ancestors_of_b.end();
}
...
auto dom::detail::tree_internals::length(
        ...
        -> ext::number<ulong>
{
    if (ext::multi_cast<nodes::attr*, nodes::document_type*>(node_a)) return 0;
